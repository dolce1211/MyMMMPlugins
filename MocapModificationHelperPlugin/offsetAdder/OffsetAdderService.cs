    ...
    internal class OffsetAdderService : BaseService
    {
        ...
        public override bool ExecuteInternal(int mode)
        {
            ...
            try
            {
                    ...
                    if (targetLayerFrames != null)
                    {
                        foreach (var frameTuple in targetLayerFrames)
                        {
                            if (frameTuple.frame is IMotionFrameData motionFrame)
                            {
                                ...
                                // フレームデータを更新
                                var newMotionData = new MotionData(newPosition, newRotation);
                ...
                var processItems = new List<(string layerName, MotionLayer layer, MotionFrameData frameData)>();
                ...
                foreach (var kvp in newFramesHash)
                {
                    ...
                    foreach (var tuple in kvp.Value)
                    {
                        ...
                        processItems.Add((layerName, tuple.Item1, tuple.Item2));
                    }
                }
                ...
                // 一件ずつ同期的に処理
                foreach (var layerFrameTuples in processItems.OrderBy(p => p.frameData.FrameNumber))
                {
                    try
                    {
                        // フレーム位置を移動
                    ...
                    {
                        // エラーログ出力（処理は継続）
                        System.Diagnostics.Debug.WriteLine($"フレーム処理エラー: {layerFrameTuples.layerName}, Frame:{layerFrameTuples.frameData.FrameNumber}, Error:{ex.Message}");
                    }
                }
