...
namespace Mocopi.Receiver.Core
{
    ...
    public sealed class MocopiUdpReceiver
    {
        ...
        /// Delegate type variables for achieving bone movement
        /// </summary>
        public ReceiveFrameDataEvent2 OnReceiveFrameData2 = new ReceiveFrameDataEvent2(
            (
                List<MocopiBone> boneList
            ) =>
            {
            }
        ...
        public delegate void ReceiveFrameDataEvent2(
            List<MocopiBone> boneList
        );
        ...
        { get { return this.task != null && !this.task.IsCanceled; } }
        ...
        public void UdpStart()
        {
            ...
            {
                Debug.WriteLine($"[MocopiUdpReceiver] Udp start failed. {e.GetType()} : {e.Message}");
        ...
        private async void UdpTaskAsync(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested && this.udpClient != null)
            {
                ...
                {
                    Debug.WriteLine($"[MocopiUdpReceiver] {e.Message} : {e.GetType()}");
                }
                catch (System.Exception e)
                {
                    Debug.WriteLine($"[MocopiUdpReceiver] Udp receive failed. {e.Message} : {e.GetType()}");
                    this.UdpStop();
        ...
        private void HandleSonyMotionFormatData(byte[] message)
        {
            ...
            else if (SonyMotionFormat.IsFrameDataBytes(bytesSize, message))
            {
                ...
                ))
                {
                    ...
                    var bones = new List<MocopiBone>();
                    if (SonyMotionFormat2.IsFrameDataBytes(bytesSize, message))
                    {
                        if (SonyMotionFormat2.ConvertBytesToFrameData(
                            bytesSize,
                            message,
                            out ulong ulongSenderIp2,
                            out int senderPort2,
                            out int frameid2,
                            out float timestamp2,
                            out bones
                            ))
                        {
                            this.SetSkeletonData2(ULongToIpAddressString(ulongSenderIp),
                                senderPort,
                                frameid,
                                timestamp,
                                bones);
        ...
        private void SetSkeletonData2(string senderIp, int senderPort,
            int frameid, float timestamp, List<MocopiBone> boneList)
        {
            this.OnReceiveFrameData2?.Invoke(boneList);
        }
